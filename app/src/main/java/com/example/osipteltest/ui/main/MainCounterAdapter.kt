...
class MainCounterAdapter (private val list: List<Counter>, private val itemClickListener: CounterListClickListener): RecyclerView.Adapter<BaseViewHolder<*>>(){
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BaseViewHolder<*> {
        val itemBinding = ItemRvMainCounterBinding.inflate(LayoutInflater.from(parent.context), parent, false)

        val holder = CounterViewHolder(itemBinding, parent.context)

        itemBinding.root.setOnClickListener {
            val position =
                holder.adapterPosition.takeIf { it != DiffUtil.DiffResult.NO_POSITION }?: return@setOnClickListener
...
//    }
    ...
    private inner class CounterViewHolder(val binding: ItemRvMainCounterBinding, val context: Context): BaseViewHolder<Counter>(binding.root) {
